  ...
  useEffect(() => {
    ...
    const tryRemove = () => {
      if (splineReady && projectsReady) {
        try {
          console.log("prevent-scroll-client: removing spline-ssr-hidden (both ready)");
          ...
          try {
            ...
            console.warn("prevent-scroll-client: dispatch mount-spline failed", e);
          }
        } catch (e) {
    ...
    const fallback = setTimeout(() => {
      try {
        console.log("prevent-scroll-client: fallback removing spline-ssr-hidden");
        document.documentElement.classList.remove("spline-ssr-hidden");
        try {
          window.dispatchEvent(new Event("mount-spline"));
          console.log("prevent-scroll-client: fallback dispatched mount-spline");
        } catch (e) {
